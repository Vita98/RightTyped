...
class KeyboardViewController: UIInputViewController {
    ...
    override func viewDidLoad() {
        ...
        if !UserDefaultManager.shared.isFirstBootForExtension() && !(UserDefaultManager.shared.isProPlanExpired() || UserDefaultManager.shared.hasProPlanJustBeenDisabled()){
    ...
    private func setInterfaceHeight(fromWillTransition : Bool = false){
        ...
        }else if UserDefaultManager.shared.isProPlanExpired() || UserDefaultManager.shared.hasProPlanJustBeenDisabled(){
            //set the maximum height to fit the view with the error message
            contentViewHightConstraint = contentView.heightAnchor.constraint(equalToConstant: size.height / 2)
